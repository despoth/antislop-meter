...
Шкала **0–10, где 10 — слопа нет**. Целевая планка — 8,5. Нижний якорь откалиброван на живом объекте: разобранный по кодам лендинг на 1,5 лежит в [`references/example-slop.md`](references/example-slop.md).
...
| [`references/criteria.md`](references/criteria.md) | каталог: 100 критериев по 10 осям, у каждого признак, вес и чем заменить |
...
| [`references/example-slop.md`](references/example-slop.md) | эталон на 1,4 из 10 с разбором каждой находки |
...
Поверх арифметики работает потолок по плотности: три-четыре тяжёлых находки — оценка не выше 3,0, пять и больше — не выше 1,5. Мягкие веса нужны, чтобы различать оттенки хорошей работы; странице, собранной из одних дефолтов, оттенки не нужны, там нужен диагноз.

Поверх потолка работает противовес, максимум +1,5: по +0,5 за собственную съёмку, за проверяемую фактуру (нормы, расход, сроки) и за честную оговорку о том, чего продукт не может. Слоп — это отсутствие решений, поэтому за наличие своего инструмент добавляет.
